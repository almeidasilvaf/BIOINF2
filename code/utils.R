...
#'
#' @return A list of character vectors. List names indicate BUSCO IDs,
#' and vectors inside list elements indicate species that contain such BUSCO
#' genes.
#'
get_busco_representation <- function(outdir) {
    
    species <- dir(outdir)
    species <- species[grepl("\\.fa$|\\.fasta$", species)]
    ...
    names(busco_rep) <- gsub("\\.fa$|\\.fasta$", "", species)
    ...
    species_per_busco <- lapply(species_per_busco, function(x) {
        y <- gsub("[0-9]*$", "", x)
    })
...
read_busco_sequences <- function(
    ...
    names(busco_seqs) <- gsub("\\.fa|\\.fasta", "", species)
    
    # Create a list of AAStringSet objects, one for each busco gene
    if(verbose) { message("Combining sequences from the same BUSCO family...") }
    busco_combined <- lapply(selected_buscos, function(x) {
        cseq <- lapply(busco_seqs, `[`, x)
        cseq <- Reduce(c, unlist(cseq))
        
        return(cseq)
    })
    names(busco_combined) <- selected_buscos
    
    return(busco_combined)
}
...
#' Default: 2
#' 
align_sequences <- function(seq_list, threads = 1) {
...
#' Infer a single-locus trees from many MSAs with IQ-TREE 2
#' 
#' @param aln_seqlist A list of `AAStringSet` objects.
#' 
infer_singlelocus_trees <- function(
        aln_seqlist, outgroup, threads = 2,
        aln_dir = NULL,
        outdir = NULL
) {
    
    # Define directories with MSAs and trees
    if(is.null(aln_dir)) {
        aln_dir <- file.path(tempdir(), "aln_dir")
    }
    if(is.null(outdir)) {
        outdir <- file.path(tempdir(), "treedir") 
    }
    if(!dir.exists(aln_dir)) { dir.create(aln_dir, recursive = TRUE) }
    if(!dir.exists(outdir)) { dir.create(outdir, recursive = TRUE) }
    
    # Write FASTA files
    wt <- lapply(seq_along(aln_seqlist), function(x) {
        
        locus <- paste0("locus_", names(aln_seqlist)[x])
        message("Inferring tree for ", locus)
        
        # Write FASTA files to a directory
        aln_dir <- file.path(tempdir(), "aln_dir")
        if(!dir.exists(aln_dir)) { dir.create(aln_dir, recursive = TRUE) }
        
        Biostrings::writeXStringSet(
            aln_seqlist[[x]], 
            filepath = file.path(aln_dir, paste0(locus, ".fa"))
        )
    })
